...
class PublixDataProcessor:
    ...
    def parse_publix_date(self, date_str: str) -> Optional[date]:
    ...
    def parse_publix_time(self, date_str: str) -> Optional[time]:
        ...
        for line in lines:
            ...
            if re.match(r'.*\d+\.\d{2}\s*[THFP]?\s*$', line):
                ...
                if len(parts) >= 2:
                    ...
                    tax_flag = parts[-1] if len(parts) > 2 and not re.match(r'\d+\.\d{2}', parts[-1]) else ''
            ...
            # Look for savings
                ...
                if savings_match:
                    receipt_info['savings']['digital_coupon'] = self.parse_decimal(savings_match.group())
                ...
                try:
                    ...
                    if next_line_idx < len(lines):
                        ...
                        if savings_match:
                            receipt_info['savings']['total_savings'] = self.parse_decimal(savings_match.group())
            ...
            # Look for payment info
                ...
                for i in range(max(0, line_idx-5), min(len(lines), line_idx+10)):
                    ...
                    if 'Amount:' in payment_line:
                        ...
                        if amount_match:
                            receipt_info['payment_info']['amount'] = self.parse_decimal(amount_match.group(1))
                    elif 'Auth #:' in payment_line:
                        receipt_info['payment_info']['auth_number'] = payment_line.split('Auth #:')[-1].strip()
                    elif 'Acct #:' in payment_line:
                        receipt_info['payment_info']['account_number'] = payment_line.split('Acct #:')[-1].strip()
            ...
            # Look for FSA info
                ...
                fsa_match = re.search(r'(\d+\.\d{2})', line)
                if fsa_match:
                    receipt_info['fsa_info']['prescription'] = self.parse_decimal(fsa_match.group(1))
            elif 'Non-Prescription (H):' in line:
                fsa_match = re.search(r'(\d+\.\d{2})', line)
                if fsa_match:
                    receipt_info['fsa_info']['non_prescription'] = self.parse_decimal(fsa_match.group(1))
    ...
    def process_detail_file(self, filepath: str) -> Optional[Dict]:
    ...
    def process_raw_to_yaml(self) -> int:
        ...
        detail_files = [f for f in os.listdir(self.raw_dir) 
    ...
    def load_yaml_to_database(self) -> int:
        ...
        print(f"   🗃️  Data loaded to: publix_purchases table")
    ...
    def load_purchase_to_database(self, db: GroceryDB, purchase_data: Dict) -> int:
        ...
        try:
            ...
            purchase_date = self.parse_publix_date(purchase_data.get('purchase_date')) if purchase_data.get('purchase_date') else None
            purchase_time = None
            if purchase_data.get('purchase_time'):
                try:
                    purchase_time = datetime.strptime(purchase_data.get('purchase_time'), '%H:%M:%S').time()
                except:
                    pass
            ...
            for product in products:
                item_record = self.build_item_record(
                    transaction_number, receipt_id, purchase_date, purchase_time,
                    store_name, store_address, store_manager, store_phone,
                    order_total, tax_amount, grand_total, vendor_coupon_amount, store_coupon_amount,
                    digital_coupon_savings, total_savings,
                    payment_amount, payment_auth_number, payment_account_number,
                    fsa_total, fsa_prescription_amount, fsa_non_prescription_amount,
                    cashier_name, supervisor_number,
                    product, None, purchase_data
                )
            ...
            for receipt_item in receipt_items:
                item_record = self.build_item_record(
                    ...
                    fsa_total, fsa_prescription_amount, fsa_non_prescription_amount,
                    cashier_name, supervisor_number,
                    None, receipt_item, purchase_data
                )
    ...
    def build_item_record(self, transaction_number, receipt_id, purchase_date, purchase_time,
                         store_name, store_address, store_manager, store_phone,
                         order_total, tax_amount, grand_total, vendor_coupon_amount, store_coupon_amount,
                         digital_coupon_savings, total_savings,
                         payment_amount, payment_auth_number, payment_account_number,
...
if __name__ == "__main__":
    ...
    if yaml_count > 0:
        ...
        if db_count > 0:
            ...
            print(f"   🗃️  Publix data collection system ready!")
        else:
