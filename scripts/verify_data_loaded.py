...
class DataVerifier:
    ...
    def verify_all_tables(self):
        ...
        for table_name, retailer_name in tables:
            ...
            if stats["status"] == "success":
                ...
                print("   ⚠️  0 records (table exists but empty)")
            ...
            for table_name, retailer_name in tables:
                try:
                    ...
                    if result and result[0]:
                        ...
                        freshness_results.append(
                            {"retailer": retailer_name, "latest_date": None, "days_ago": None}
                        )
                    ...
                    freshness_results.append({"retailer": retailer_name, "error": str(e)})
    ...
    def print_summary(
        self, verification_results, freshness_results, total_records, successful_tables
    ):
        ...
        print(
            f"   📅 Tables with recent data (≤7 days): {recent_data_count}/{len(freshness_results)}"
        )
...
def main():
    ...
    parser.add_argument(
        "--detailed", action="store_true", help="Show detailed statistics for each table"
    )
    parser.add_argument(
        "--recent", action="store_true", help="Show only recent data (last 30 days)"
    )
    parser.add_argument("--export", action="store_true", help="Export verification results to JSON")
    ...
    verifier = DataVerifier(
        environment=environment, detailed=args.detailed, recent_only=args.recent
    )
    ...
    verifier.print_summary(
        verification_results, freshness_results, total_records, successful_tables
    )
