    ...
    try:
        ...
        for item in items:
            ...
            db_item = {
                ...
                "fuel_quantity": safe_float(fuel_info.get('fuel_unit_quantity', 0)) if is_fuel else None,
                "fuel_grade": fuel_info.get('fuel_grade_description', '') if is_fuel else None,
                "fuel_unit_price": safe_float(item.get('item_unit_price_amount', 0)) if is_fuel else None,
                "payment_method": ', '.join([p.get('tender_type_name', p.get('tender_description', '')) for p in payments]),
...
def load_all_yaml_files():
    ...
    if not os.path.exists(data_dir):
        ...
        print(f"💡 Run the Costco scraper first to generate YAML files")
        return False
    ...
    if successful_files > 0:
        ...
        print(f"🔍 You can query it using the grocery_db module")
