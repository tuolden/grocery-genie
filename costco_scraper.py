...
def get_warehouse_receipts(months_back=3):
    ...
    if BEARER_TOKEN == "PASTE_YOUR_BEARER_TOKEN_HERE" or CLIENT_ID == "PASTE_YOUR_CLIENT_ID_HERE":
    ...
    receipts{
      ...
      membershipNumber
        ...
        if response.status_code == 200:
            ...
            print(f"💾 Raw response saved to: warehouse_receipts_response.json")
            ...
            if 'data' in data and 'receiptsWithCounts' in data['data']:
                ...
                if receipts:
                    ...
                    print(f"📁 Location: ./data/costco/")
                    print(f"💡 Ready for database import!")

                    return True, data
                else:
                    print(f"⚠️ No receipts found for the specified period")
                    return True, data
            ...
            print(f"💡 Your token has expired. Get a fresh one:")
            print(f"   1. Go to costco.com > My Account > Orders and Purchases")
            print(f"   2. Click on 'Warehouse' tab")
            print(f"   3. Open Dev Tools (F12) > Network tab")
            print(f"   4. Refresh the page")
            print(f"   5. Find 'graphql' request")
            print(f"   6. Copy the new Bearer token and Client ID")
            print(f"   7. Update BEARER_TOKEN and CLIENT_ID in this file")
            return False, None
        else:
...
if __name__ == "__main__":
    ...
    success, data = get_warehouse_receipts(months_back=3)

    if success:
        print(f"\n🎉 SCRAPING COMPLETED SUCCESSFULLY!")
        print(f"💡 Next step: Run the database loader to import YAML files")
    else:
        print(f"\n❌ Scraping failed")
        print(f"🔄 Check token setup and try again")

    print(f"\n📝 Remember: Tokens expire every 15-30 minutes")
    print(f"🔄 Update tokens when needed")
