...
from grocery_db import GroceryDB
...
class ReceiptMatcher:
    ...
    def find_matches(self, purchases: List[PurchaseItem], list_items: List[ListItem]) -> List[MatchResult]:
        ...
        for purchase in purchases:
            ...
            if best_match:
                if best_match.store == purchase.store:
                    action = 'mark_checked'
                    logger.info(f"✅ SAME STORE MATCH: {purchase.item_name} → {best_match.item_name} (score: {best_score:.2f})")
                else:
                    action = 'remove_from_other_lists'
                    logger.info(f"🔄 CROSS STORE MATCH: {purchase.item_name} → {best_match.item_name} (score: {best_score:.2f})")
            else:
                action = 'no_action'
    ...
    def _mark_item_checked(self, cur, match: MatchResult):
        ...
        logger.info(f"✅ MARKING CHECKED: {match.list_item.item_name} in {match.list_item.table_source}")
    ...
    def _remove_from_other_lists(self, cur, match: MatchResult):
        ...
        for store, table_name in self.list_tables.items():
            ...
            """, (f"%{match.purchase_item.item_name}%", f"%{match.purchase_item.item_name}%"))
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Receipt Matcher - Mark purchased items from receipts')
