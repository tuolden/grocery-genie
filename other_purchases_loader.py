...
class OtherPurchasesLoader:
    ...
    def _upsert_purchase_item(self, cur, item_data: Dict, store_name: str, 
        ...
        try:
            ...
            if price is not None:
                try:
                    ...
                    logger.warning(f"⚠️  Invalid price '{price}' for item '{item_name}', setting to NULL")
                    price = None
    ...
    def process_yaml_file(self, file_path: Path) -> bool:
        ...
        try:
            ...
            for item in items:
                if self._upsert_purchase_item(cur, item, store_name, purchase_date, 
                                            purchase_time, yaml_data):
                    success_count += 1
            
            conn.commit()
            
            logger.info(f"✅ Successfully processed {success_count}/{len(items)} items from {file_path.name}")
...
def main():
    ...
    parser.add_argument('--data-dir', type=str, default='./data/other',
