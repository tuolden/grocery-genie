    ...
    def run_unit_tests(self):
        ...
        try:
            ...
            result = subprocess.run(
                [sys.executable, str(unit_test_script)],
                capture_output=True,
                text=True,
                timeout=120
            )
    ...
    def run_integration_tests(self):
        ...
        try:
            ...
            if result.stdout:
                ...
                for line in lines:
                    if any(keyword in line for keyword in ['✅', '❌', 'PASSED', 'FAILED', 'SUMMARY']):
                        logger.info(f"📝 {line}")
    ...
    def run_smoke_tests(self):
        ...
        try:
            ...
            if result.stdout:
                lines = result.stdout.split('\n')
                for line in lines:
                    if any(keyword in line for keyword in ['✅', '❌', 'PASSED', 'FAILED', 'SUMMARY', 'TOTAL:']):
                        logger.info(f"📝 {line}")
